...
public class HardwareAddressPropertyDelegate extends PropertyDelegate {
    ...
    private byte[] getHardwareAddressRaw() {
        LOGGER.entering(CLASS_NAME, "getHardwareAddressRaw");
        ...
        try {
            ...
            if (haddr == null) {
                LOGGER.logp(Level.WARNING, CLASS_NAME, "getHardwareAddressRaw", "cannot get hardware address: " + networkInterface);
            }
        } catch (SocketException ex) {
            LOGGER.logp(Level.WARNING, CLASS_NAME, "getHardwareAddressRaw", "cannot get hardware address: ", ex);
        }
        
        LOGGER.exiting(CLASS_NAME, "getHardwareAddressRaw", haddr);
        return haddr;
    }
    
    private byte[] getHardwareAddressString() {
        LOGGER.entering(CLASS_NAME, "getHardwareAddressString");
        
        byte[] haddr = getHardwareAddressRaw();
        
        if (haddr == null) {
            LOGGER.exiting(CLASS_NAME, "getHardwareAddressString", null);
            return null;
        }
        
        StringBuilder builder = new StringBuilder();
        for (int i=0; i<haddr.length; i++) {
            builder.append(String.format("%02x", 0x00ff & haddr[i]));
        }
        
        byte[] bytes = builder.toString().getBytes();
        LOGGER.exiting(CLASS_NAME, "getHardwareAddressString", bytes);
        return bytes;
    }
    ...
    public ObjectData getHardwareAddressData() {
        ...
        if (isStringMode) {
            bytes = getHardwareAddressString();
        } else {
            bytes = getHardwareAddressRaw();
        }
